       ...
       <div class="event-list-page-wrap py-5">
            <div class="container">
                <div class="event-list-search shadow">
                    <form action="<?=base_url('search/my-event')?>" Method="GET">
                        <div class="row g-0">
                            <div class="col-md-5">
...
function getMoreData(lastId) {
      $(window).off("scroll");
    $.ajax({
        url: '<?php echo base_url(); ?>event/loadMyevent?lastId=' + lastId,
        type: "get",
        beforeSend: function ()
        {
            $('.ajax-loader').show();
...
function deleteMyevent(id) 
{
	swal({
		...
		if (isConfirm) {
			window.location.href = '<?= base_url('event/deleteMyevent/') ?>'+id
		}
	});
}
</script>		