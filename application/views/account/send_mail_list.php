									...
									?>
											<tr id="remove_row_<?=$v->id?>">
												...
												<td>
                                                    ...
                                                    <a href="javascript:void(0);"  title="Delete"  onclick="deleteSendMail(<?=$v->id?>)"><i class="fa fa-trash"></i> </a>	
...
function get_recipients(id){
	$.ajax({
		type: "POST", 
		url:  '<?= base_url('email/get_recipients/') ?>',  
		data: {id:id}, 
		beforeSend: function(){
		},
		success: function(response){
...
function deleteSendMail(id)
{
	...
	if (isConfirm) {
			$.ajax({
				type: "POST", 
				url:  '<?= base_url('mailer/delete_send_list/') ?>',  
				data: {id:id}, 
				dataType : 'json',
				beforeSend: function(){
				},
