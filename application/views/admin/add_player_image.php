...
$(document).ready(function(){
	...
	$("#submitAboutus").on('submit', function(e){
		...
		$.ajax({
		type: 'POST',
		url: '<?php echo base_url('admin/gallery/submitImageGallery'); ?>',
		data: form_data,
		dataType:"json",
		contentType: false,
		cache: false,
		...
		success: function(data){
			// var output = $(".uploadinfobox");
			 ...
			 if(data.status == 1)
			 {
				 swal({title: "Sucess!", text: "<strong>"+data.message+"</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = "<?=base_url('admin/gallery/player_gallery/'.@$this->uri->segment(4).'')?>"});
			 }
				// if(data.email_error != '')
				// {
