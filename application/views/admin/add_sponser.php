						...
						<div class="row">
							<div class="col-sm-8">
								...
								<div class="form-group mb-2 files">
									...
									<input type="hidden" class="form-control" name="user_id"  id="user_id"  value="<?php echo !empty($this->uri->segment(4)) ? $this->uri->segment(4) : ''; ?>">
								</div>
...
$(document).ready(function(){
	$("#submitAboutus").on('submit', function(e){
		...
		// if(totalfiles == 0){
			 // $('#image_error').html('Please select the images.');
			 // return false;
		// }
		
		for (var index = 0; index < totalfiles; index++) {
		    form_data.append("image[]", document.getElementById('upload_image').files[index]);
		}
		...
		$.ajax({
		type: 'POST',
		url: '<?php echo base_url('admin/sponser/addsponser'); ?>',
		data: form_data,
		dataType:"json",
		contentType: false,
		cache: false,
