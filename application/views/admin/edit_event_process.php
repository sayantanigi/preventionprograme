                     ...
                     <div class="card-body">    
                        <form id="submitform" method="post" enctype="multipart/form-data" >
							...
							<div class="form-group mb-2">
                                ...
                                <textarea type="text" class="form-control editor summermote" name="description"  id="description"  autocomplete="off"><?=@$event_process->description?></textarea>
								<input type="hidden"  name="id"  id="id" value="<?=$event_process->id?>" >
                            </div>
							...
							<img src="<?=(!empty($event_process->image) ? base_url('uploads/event_process/'.$event_process->image.'') : base_url('uploads/noimage.jpg'))?>" style="width: 30%;height: 150px;border: 4px solid #cdcdcd;border-radius: 10px;" id="blahImg" ><br/><br/>
							
							 <div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Status</label>
                                <select class="form-control" name="status"  id="userstatus">
                                    <option value="">Select Status</option>
                                    <option value="1" <?=($event_process->status == 1) ? 'selected' : ''?>>Active</option>
                                    <option value="0" <?=($event_process->status == 0) ? 'selected' : ''?>>Inactive</option>
                                </select>
                            </div>
...
$(document).ready(function(){
	$("#submitform").on('submit', function(e){
		...
		$.ajax({
		type: 'POST',
		url: '<?php echo base_url('admin/cms/edit_stepProcess'); ?>',
