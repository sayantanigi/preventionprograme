                     ...
                     <div class="card-body">    
                        <form id="submitform" method="post" enctype="multipart/form-data" >
                            <div class="form-group mb-2">
                                ...
                                <input type="text" class="form-control" name="heading"  id="heading1"  autocomplete="off" value="<?=@$feature->heading?>">
							...
							<div class="form-group mb-2">
                                ...
                                <textarea type="text" class="form-control editor summermote" name="description"  id="description"  autocomplete="off"><?=@$feature->description?></textarea>
                            </div>
							...
							<img src="<?=!empty(@$feature->image) ? base_url('uploads/feature/'.@$feature->image.'') : base_url('uploads/noimage.jpg')?>" style="width: 30%;height: 150px;border: 4px solid #cdcdcd;border-radius: 10px;" id="blahImg" ><br/><br/>
							
							 <div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Status</label>
                                <select class="form-control" name="status"  id="userstatus">
                                    <option value="">Select Status</option>
                                    <option value="1" <?php echo (@$feature->status == 1) ? 'selected' : ''?>>Active</option>
                                    <option value="0" <?php echo (@$feature->status == 0) ? 'selected' : ''?>>Inactive</option>
                                </select>
                            </div>
...
$(document).ready(function(){
	$("#submitform").on('submit', function(e){
		...
		$.ajax({
		type: 'POST',
		url: '<?php echo base_url('admin/feature/editFeature'); ?>',
		...
		success: function(data){
			if(data.status == 1){
				swal({title: "Sucess!", text: "<strong>"+data.message+"</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = "<?=base_url('admin/feature')?>"});
