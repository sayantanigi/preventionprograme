                     ...
                     <div class="card-body">    
                        <form id="submitform" method="post" enctype="multipart/form-data" > 
                            <div class="form-group mb-2">
                                ...
                                <input type="text" class="form-control" name="game_name"  id="game_name"  autocomplete="off" value="<?php echo @$games->game_name; ?>">
                            </div>
							<small id="game_name_error"></small>
							
							<div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Game Description</label>
                                <textarea type="text" class="form-control editor summermote" name="game_description"  id="game_description"  autocomplete="off"><?php echo @$games->game_description; ?></textarea>
                            </div>
							...
							<!--<div class="form-group mb-2">
                                ...
                                <select class="form-control" name="game_type"  id="game_type">
                                    ...
                                    <option value="One to One" <?php echo (@$games->game_type == 'One to One') ? 'selected' : '' ; ?>>One to One</option>
                                    <option value="Tournament" <?php echo (@$games->game_type == 'Tournament') ? 'selected' : '' ; ?>>Tournament</option>
                                </select>
                            </div>
							<small id="game_type_error"></small>-->
							
							<!--<div class="form-group mb-2" id="participant_teams" style="<?php echo (@$games->game_type == 'Tournament') ? 'display:block' : 'display:none' ; ?>">
                                <label class="fw-semibold  text-black">No. of Participant Teams</label>
								<input type="number" class="form-control" name="no_teams"  id="no_teams" autocomplete="off" value="<?php echo @$games->no_participant_teams; ?>">
                            </div>
							<small id="noTeams"></small>-->
							
							<div class="form-group mb-2" id="tournament_start_date" >
                                <label class="fw-semibold  text-black"> Date</label>
								<input type="date" class="form-control" name="t_startdate"  id="t_startdate" autocomplete="off" value="<?php echo !empty($games->tournament_start_date) ? strftime('%Y-%m-%d',strtotime($games->tournament_start_date)) : ''; ?>">
                            </div>
							<small id="Tstartdate"></small>
							
							<!--<div class="form-group mb-2" id="tournament_end_date" style="<?php echo (@$games->game_type == 'Tournament') ? 'display:block' : 'display:none' ; ?>">
                                <label class="fw-semibold  text-black">Tournament End Date</label>
								<input type="date" class="form-control" name="t_enddate"  id="t_enddate" autocomplete="off" value="<?php echo !empty($games->tournament_end_date) ? strftime('%Y-%m-%d',strtotime($games->tournament_end_date)) : ''; ?>">
                            </div>
							<small id="Tenddate"></small>-->
							<?php
								$selectedSportId= array();
								$exSportId = explode(',', $games->sport_id);
								foreach ($exSportId as $sportId){
								$selectedSportId[]= $sportId;
								}
								//print_r($selectedSportId);
							?>
							<div class="form-group mb-2" style="<?= !empty(@$games->game_type == 'One to One') ? 'display:block' : 'display:none'; ?>" id="single_sports">
							...
							<div class="form-group mb-2" style="<?= !empty(@$games->game_type == 'Tournament') ? 'display:block' : 'display:none'; ?>" id="multiple_sports">
                                <label class="fw-semibold  text-black">Sport</label>
							...
							<?php
								$sql = $this->db->query("select id, first_name, last_name from users where id = ".$games->organizer_id."")->result();
							?>
							...
							<div class="form-group mb-2">
                                ...
                                <select class="form-control" name="organizer"  id="organizer" >
									   ...
									   <?php if (is_array($sql) || is_object($sql)) { ?>
									   <?php foreach ($sql as $key => $v): ?>
									      <option value="<?php echo @$v->id; ?>" <?php echo (@$games->organizer_id == @$v->id) ? 'selected' : '' ; ?>><?php echo @$v->first_name; ?> <?php echo @$v->last_name; ?></option>
									   <?php endforeach; }?>
                                </select>
							...
							<div class="form-group mb-2">
								...
								<input type="text" class="form-control" name="address"  id="autocomplete" autocomplete="off" value="<?php echo @$games->venue; ?>">
                            </div>
							...
							<input type="hidden" class="form-control" name="id"  id="id" value="<?php echo @$games->id; ?>">
                            ...
                            <div class="row">
                                <div class="container">
										...
										<div class="profile clearfix">                            
											...
											<div class="user clearfix">
												...
												<div class="avatar item" id="item">
													<img src="<?= !empty(@$games->game_image) ? base_url('uploads/games/'.@$games->game_image.'') : base_url('uploads/unnamed.jpg'); ?>" class="img-thumbnail img-profile" id="blah">
												</div>   
								 ...
								 <div class="col-lg-12 mb-3">
									...
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Game Name</h6></label><p class="text-muted" id="gamename"><?php echo @$games->game_name; ?></p></div>
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Game Description</h6></label><p class="text-muted" id="gamedesc"><?php echo @$games->game_description; ?></p></div>
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Date</h6></label><p class="text-muted" id="datestart"><?php echo !empty($games->tournament_start_date) ? strftime('%Y-%m-%d',strtotime($games->tournament_start_date)) : ''; ?></p></div>
									
									<?php
									   $sports = $this->db->query("select * from sports where id = ".@$games->sport_id."")->row();
									   $orgInfo = $this->db->query("select first_name, last_name from users where id = ".@$games->organizer_id."")->row();
									   
									?>
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0"><h6>Sport</h6></label><p class="text-muted" id="sport_new"><?=@$sports->sports_name; ?></p></div>
									
								
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0"><h6>Organizer</h6></label><p class="text-muted" id="organizer_new"><?=@$orgInfo->first_name?> <?=@$orgInfo->last_name?></p></div>
									...
									<p id="coach_address"><?php echo @$games->venue; ?></p>
...
$(document).ready(function(){
	$("#submitform").on('submit', function(e){
		...
		$.ajax({
		type: 'POST',
		url: '<?php echo base_url('admin/games/editGames'); ?>',
