                     ...
                     <div class="card-body">    
                        <form id="submitform" method="post" enctype="multipart/form-data" >
                            ...
                            <div class="form-group mb-2">
                                ...
                                <input type="text" class="form-control" name="fname"  id="fname" autocomplete="off" value="<?=@$guardian[0]->first_name?>">
                                <input type="hidden" name="user_id"  id="user_id"  value="<?=@$guardian[0]->id?>">
                            </div>
							<small id="fname_error"></small>
							
							<div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Last Name</label>
                                <input type="text" class="form-control" name="lname"  id="lname" autocomplete="off" value="<?=@$guardian[0]->last_name?>">
                            </div>
							<small id="lname_error"></small>
							
							<div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Email</label>
                                <input type="email" class="form-control" name="email"  id="email" autocomplete="off" value="<?=@$guardian[0]->email?>">
                            </div>
                            <small id="email_error"></small>
							
							<div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Phone</label>
                                <input type="number" class="form-control" name="phone"  id="phone" autocomplete="off" value="<?=@$guardian[0]->phone?>">
                            </div>
							<small id="phone_error"></small>
							
							<div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Address</label>
								<input type="text" class="form-control" name="address"  id="autocomplete" autocomplete="off" value="<?=@$guardian[0]->address?>">
								<input type="hidden" placeholder="Near"  name="latitude" id="latitude" value="<?=@$guardian[0]->latitude?>">
			                    <input type="hidden" placeholder="Near" name="longitude" id="longitude" value="<?=@$guardian[0]->longitude?>">
                            </div>
							...
							<div class="form-group mb-2">
                                ...
                                <select class="form-control" name="status" id="userstatus">
                                    ...
                                    <option value="1" <?=(@$guardian[0]->status == 1) ? 'selected' : '' ?>>Active</option>
                                    <option value="0" <?=(@$guardian[0]->status == 0) ? 'selected' : '' ?>>Inactive</option>
                                </select>
                            </div>
                            ...
                            <div class="row">
                                <div class="container">
										...
										<div class="profile clearfix">                            
											...
											<div class="user clearfix">
												<div class="avatar item" id="item">
													<img src="<?= !empty(@$guardian[0]->profile_image) ? base_url('uploads/guardian/'.@$guardian[0]->profile_image.'') : base_url('uploads/unnamed.jpg'); ?>" class="img-thumbnail img-profile" id="blah">
												</div>                                
								 ...
								 <div class="col-lg-12 mb-3">
									...
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>First Name</h6></label><p class="text-muted" id="first_name"><?=@$guardian[0]->first_name?> <?=@$guardian[0]->last_name?></p></div>
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Last Name</h6></label><p class="text-muted" id="last_name"><?=@$guardian[0]->last_name?></p></div>
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Email</h6></label><p class="text-muted" id="coach_email"><?=@$guardian[0]->email?></p></div>
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Phone</h6></label><p class="text-muted" id="coach_phone"><?=@$guardian[0]->phone?></p></div>
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Status</h6></label><p class="text-muted" id="coach_status"><?=(@$guardian[0]->status == 1) ? 'Active' : 'Inactive'?></p></div>
									
									<div class="d-flex align-items-center justify-content-between mb-2">
									<h6 class="card-title mb-0">Address</h6>
									</div>
									<p id="coach_address"><?=@$guardian[0]->address?></p>
									</div>
									</div>
                                </div>
                            </div>
...
$(document).ready(function(){
	$("#submitform").on('submit', function(e){
		...
		$.ajax({
		type: 'POST',
		url: '<?php echo base_url('admin/guardian/editguardian'); ?>',
		data: form_data,
		dataType:"json",
		contentType: false,
		cache: false,
	...
	$('#crop').click(function(){
		...
		canvas.toBlob(function(blob){
			...
			reader.onloadend = function(){
				...
				$.ajax({
					url:'<?php echo base_url("admin/guardian/cropImage")?>',
					method:'POST',
					...
					success:function(data)
					{
						$modal.modal('hide');
						console.log(data)
						$('#item img').attr('src', '<?php echo base_url(); ?>uploads/guardian/' + data);
						$('#profileImg').val(data);
					}
				});
			};
			...
			reader.onloadend = function(){
				...
				$.ajax({
					...
					success:function(data)
					{
						$modal.modal('hide');
						console.log(data)
						$('#Cover-Image img').attr('src', '<?php echo base_url(); ?>uploads/guardian/' + data);
						$('#coverImg').val(data);
					}
				});
			};
