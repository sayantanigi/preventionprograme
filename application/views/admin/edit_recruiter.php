                     ...
                     <div class="card-body">    
                        <form id="submitform" method="post" enctype="multipart/form-data" >
                            ...
                            <div class="form-group mb-2">
                                ...
                                <input type="text" class="form-control" name="fname"  id="fname" required autocomplete="off" value="<?=@$user->first_name; ?>">
                            </div>
							
							<div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Last Name</label>
                                <input type="text" class="form-control" name="lname"  id="lname" required autocomplete="off" value="<?=@$user->last_name; ?>">
                            </div>
							...
							<div class="form-group mb-2">
								...
								<input type="text" class="form-control" name="address"  id="autocomplete" required autocomplete="off" value="<?=@$user->address; ?>">
...
$(document).ready(function(){
	$("#submitform").on('submit', function(e){
		...
		$.ajax({
		type: 'POST',
		url: '<?php echo base_url('admin/users/editRecruiter'); ?>',
		data: form_data,
		dataType:"json",
		contentType: false,
		cache: false,
...
$(document).on('keyup','#fname',function(e){
	...
	$(document).on('change','#userstatus',function(e){
        ...
        if(status == 1){
          $("#recruiter_status").text('Active');
        }
		if(status == 0){
          $("#recruiter_status").text('Inactive');
        }
    });
...
<script>
	...
	function initialize() {
		...
		autocomplete.addListener('place_changed', function() {
			...
			$("#recruiter_address").text(address);
			...
			reader.onloadend = function(){
				...
				$.ajax({
					...
					success:function(data)
					{
						...
						$('#itemImage img').attr('src', '<?php echo base_url(); ?>uploads/profile_image/' + data);
