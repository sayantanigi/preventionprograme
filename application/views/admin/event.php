								   ...
								   <?php if (is_array($event) || is_object($event)) { ?>
										<?php 
											foreach ($event as $key => $v): 
												if(@$v->user_id == 0){
													...
													$getUser = $this->db->query("select fname, lname from users where id = ".@$v->user_id."")->row();
										...
										?>
											...
											<tr>
												<td><?= $key+1 ?></td>
												<td><?=$userName?></td>
												<td><?=ucfirst(@$v->event_name); ?></td>
												<td style="width:10%;" class="tableDes"><p class="comment more tableDes"><?=strip_tags(@$v->event_description); ?></p></td>
												<td><?=ucfirst(@$v->event_address); ?></td>
												<td><?=ucfirst(@$v->event_price); ?></td>
												<td>
													<div class="form-check mb-3 mt-3">
														<input type="checkbox" class="form-check-input small" id="statusChange_<?=$key?>" switch="bool" value="<?= @$v->status ?>" <?= (@$v->status == 1)? 'checked':'' ?>  onchange="changeDealStatus(<?=@$v->event_id?>, $(this))">
												...
												<td class="text-center">
													...
													<a href="<?= base_url('admin/event/edit/'.$v->event_id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
													    <i class="fas fa-edit"></i>
													</a>
													
												    <a href="<?= base_url('admin/event/view/'.@$v->event_id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="View">
														<i class="fa fa-eye"></i>
													</a>
													
													<a href="<?= base_url('admin/event/invited_people/'.$v->event_id) ?>" class="btn btn-outline-primary btn-sm" data-toggle="tooltip" title="Invited People">
													    <img src="<?=base_url('uploads/icon/invite-icon-20.jpg')?>" style='width: 20px;'>
													</a>
													 
													<a href="javascript:void(0)" class="btn btn-outline-warning btn-sm" data-toggle="tooltip" title="Delete"  onclick="deleteDeals(<?= @$v->event_id ?>)">
