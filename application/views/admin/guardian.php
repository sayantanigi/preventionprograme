            ...
            <div class="col-xl-12">
               <div class="card custom-shadow rounded-lg border">
                  <div class="card-body">
                  	<div class="row">
	                  	<div class="col-sm-10">
	                     	<h4 class="card-title mb-4">Gaurdian List</h4>
	                     </div>
	                     
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	 <a href="<?=base_url('admin/guardian/add')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add Guardian</a>
	                     </div>
	                  </div>   	
                     <div class="">
                        <table id="datatable" class="table table-bordered dt-responsive nowrap w-100">
								   ...
								   <?php if (is_array($guardian) || is_object($guardian)) { ?>
										<?php foreach ($guardian as $key => $v): ?>
											<tr>
												<td><?= $key+1 ?></td>
												...
												<td class="text-center">
												    ...
												    <a href="<?= base_url('admin/guardian/edit/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
														<i class="fas fa-edit"></i>
													</a>
													
													<a href="<?= base_url('admin/guardian/view/'.@$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="View">
														<i class="fa fa-eye"></i>
													</a>
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= base_url('admin/guardian/delete/') ?>'+dealId
			}
		});
	}
	...
	function changeDealStatus(id, thisSwitch) {      
		...
		$.ajax({         
			url: '<?php echo base_url('admin/guardian/changestatus'); ?>',     
			type: 'POST',       
			dataType: 'json',       
			data: {         
				userId: String(id),        
