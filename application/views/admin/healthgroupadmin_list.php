                  ...
                  <div class="card-body">
                  	<div class="row">
	                     ...
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	 <a href="<?=base_url('admin/healthgroup/add_healthgroup_admin')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add</a>
	                     </div>
	                  </div>   	
                     <div class="">
                        <table id="datatable" class="table table-bordered dt-responsive nowrap w-100">
								   <thead class="thead-light text-center">
								      <tr>
											...
											<th>Health Group Admin Name</th>
											<th>Health Group Admin Email</th>
											<th>Status</th>
								   ...
								   <?php if (is_array($result) || is_object($result)) { ?>
										<?php foreach ($result as $key => $v): ?>
											...
											<tr>
												...
												<?php
												    @$health_group_name = '';
												    if(@$v->health_group_id){
														$health_group = $this->Adminmodel->get_by('health_entity', 'single', array('id' => @$v->health_group_id), '', 1);
														if(@$health_group){
															if(@$health_group->name){
															    $health_group_name = @$health_group->name;
															}
														}
													}
												    
												?>
												<td><?=@$health_group_name; ?></td>
												...
												<td class="text-center">
												    <a href="<?= base_url('admin/healthgroup/edit_healthgroup_admin/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= base_url('admin/healthgroup/delete_healthgroup_admin/') ?>'+dealId
			}
		});
	}
	...
	function changePckStatus(id, thisSwitch) {      
		...
		$.ajax({         
			url: '<?php echo base_url('admin/healthgroup/changestatusHealthgroup_admin'); ?>',     
			type: 'POST',       
			dataType: 'json',       
			data: {         
				id: String(id),        
		...
		.done(function(data) {  
			...
			if(newStatus == 1){
				swal({title: "Sucess!", text: "<strong>Health group admin status is Activate</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = " "});
			}else if(newStatus == 0){
				swal({title: "Sucess!", text: "<strong>Health group admin status is Inctivate</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = " "});
			}
		})
		.fail(function(data) {      
			console.log(data);       
