                  ...
                  <div class="card-body">
                  	<div class="row">
	                     ...
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	 <a href="<?=base_url('admin/news/add/'.$userId.'')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add News</a>
	                     </div>
	                  </div>   	
                     <div class="">
                        <table id="datatable" class="table table-bordered dt-responsive nowrap w-100">
								   ...
								   <?php if (is_array($news) || is_object($news)) { ?>
										<?php foreach ($news as $key => $v): ?>
											<tr>
												<td ><?= $key+1 ?></td>
												<td style="width:4%;"><img src="<?=!empty(@$v->image) ? base_url('uploads/news/'.@$v->image.'') : base_url('uploads/noimage.jpg')?>" style="width:100%;"></td>
												<td ><?= @$v->heading; ?></td>
												<td ><?= @$v->description; ?></td>
												<td>
												...
												<td class="text-center">
												    ...
												    <a href="<?= base_url('admin/news/edit/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId, userId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?=base_url();?>admin/news/delete?id='+dealId+'&&uId='+userId+'';
			}
		});
	}
