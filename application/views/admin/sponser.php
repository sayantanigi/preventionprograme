            ...
            <div class="col-xl-12">
               <div class="card custom-shadow rounded-lg border">
                  <div class="card-body">
                  	<div class="row">
	                  	<div class="col-sm-10">
	                     	<h4 class="card-title mb-4">Team Coach List</h4>
	                     </div>
	                     
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	 <a href="<?=base_url('admin/sponser/add/'.$userId.'')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add Sponser</a>
	                     </div>
	                  </div>   	
                     <div class="">
                        <table id="datatable" class="table table-bordered dt-responsive nowrap w-100">
								   ...
								   <?php if (is_array($sponsers) || is_object($sponsers)) { ?>
										<?php foreach ($sponsers as $key => $v): ?>
											<tr>
												<td style="width:10%;"><?= $key+1 ?></td>
												
												<td style="width:20%;"><img src="<?=!empty(@$v->image) ? base_url('uploads/sponser/'.@$v->image.'') : base_url('uploads/noimage.jpg')?>" style="width:100%;"></td>
												
												<td class="text-center">
												
												    <a href="<?= base_url('admin/sponser/edit/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
														<i class="fas fa-edit"></i>
													</a>
													
													<a href="javascript:void(0)" class="btn btn-outline-warning btn-sm" data-toggle="tooltip" title="Delete"  onclick="deleteDeals(<?= @$v->id ?>,<?= @$userId ?>)">
...
var adminUrl = ""
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId, userId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?=base_url();?>admin/sponser/delete?id='+dealId+'&&uId='+userId+'';
			}
		});
	}
	...
	function addPlayers(dealId) 
	{
		swal({
				...
				confirmButtonColor: '#A5DC86',
				cancelButtonColor: '#DD6B55',
				confirmButtonText: 'Add Existing Player',
			...
			if (isConfirm) {
				...
				window.location.href = '<?= base_url('admin/users/add_teamplayer/') ?>'+dealId+'/?add=new-players'
			}
		});
	}
 </script>
