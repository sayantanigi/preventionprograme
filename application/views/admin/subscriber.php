                  ...
                  <div class="card-body">
                  	<div class="row">
	                     ...
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	<a href="<?=base_url('admin/subscribe/downloadCsv')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Export CSV</a>
	                     </div>
	                  </div>   	
                     <div class="">
                        <table id="datatable" class="table table-bordered dt-responsive nowrap w-100">
								   ...
								   <?php if (is_array($subscriber) || is_object($subscriber)) { ?>
										<?php foreach ($subscriber as $key => $v): ?>
										    
										    
											<tr>
												<td><?= $key+1 ?></td>
												<td><?=@$v->email; ?></td>
												<td><?=!empty(@$v->created_at) ? date('d F, Y', strtotime(@$v->created_at)) : ''?></td>
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= base_url('admin/subscribe/deleteSub/') ?>'+dealId
			}
		});
	}
