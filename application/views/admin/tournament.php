            ...
            <div class="col-xl-12">
               <div class="card custom-shadow rounded-lg border">
                  <div class="card-body">
                  	<div class="row">
	                  	<div class="col-sm-10">
	                     	<h4 class="card-title mb-4">Tournament List</h4>
	                     </div>
	                     
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	 <a href="<?=base_url('admin/tournament/add')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add Tournament</a>
	                     </div>
	                  </div>   	
                     <div class="">
                        <table id="datatable" class="table table-bordered dt-responsive nowrap w-100">
								   ...
								   <?php if (is_array($tournamentList) || is_object($tournamentList)) { ?>
										<?php foreach ($tournamentList as $key => $v): ?>
										...
										?>
										<?php
										$opponent = $this->db->query("select id, receiver_team from invite_tournament_opponent where sender_team = ".@$v->organizer_id." AND game_type='Tournament'")->row();
										?>
											<tr>
												<td><?= $key+1 ?></td>
												<td><?=@$v->tournament_name; ?></td>
												<td><?=@$v->tournament_type; ?></td>
												<td><?=ucfirst(@$organizer_id->first_name); ?> <?=ucfirst(@$organizer_id->last_name); ?></td>
												
												<td>
													<?php
													//print_r(@$opponent->receiver_team);
														if(!empty(@$opponent->receiver_team)){
															$exOpp = explode(',', @$opponent->receiver_team);
															foreach(@$exOpp as $key=>$value){
																$teamInfo = $this->db->query("select first_name, last_name, team_name from users where id = ".@$value."")->row();
																
																echo @$teamInfo->team_name.'('.ucfirst(@$teamInfo->first_name).' '.ucfirst(@$teamInfo->last_name).')'.', ';
																//print_r($teamInfo);
															}
														}
													?>
												...
												<td>
													<?php
														$opponent1 = $this->db->query("select * from invite_tournament_opponent where sender_team = ".@$v->organizer_id." AND game_type='Tournament'")->row();
												...
												<td class="text-center">
												    <a href="<?= base_url('admin/tournament/edit/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
														<i class="fas fa-edit"></i>
													</a>
													
												    <a href="<?= base_url('admin/tournament/view/'.@$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="View">
														<i class="fa fa-eye"></i>
													</a>
													
													<a href="<?= base_url('admin/tournament/invite/'.@$v->organizer_id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Invite">
...
var adminUrl = ""
   ...
   function dealDetail(orgId, Id) {
	   if(orgId){
			$.ajax({         
			url: '<?php echo base_url('admin/tournament/getwinnerOpt'); ?>',     
			type: 'POST',     
			data: {         
			orgId: orgId  
			},
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= base_url('admin/tournament/delete/') ?>'+dealId
			}
		});
	}
...
$(document).ready(function(){
	$("#submitAboutus").on('submit', function(e){
		...
		$.ajax({
		type: 'POST',
		url: '<?php echo base_url('admin/tournament/saveWinner'); ?>',
