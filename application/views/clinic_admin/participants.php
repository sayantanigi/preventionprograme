...
<div class="container-fluid m-0 Section Promotion">
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="row g-3" action="<?= base_url('clinic_admin/add_participant')?>" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-footer" style="margin: 0; border: none; padding: 0px 6px 0px 0px;">
                            <p id="resultparticipantmsg" style="margin-right: 55px; color: green; "></p>
    ...
    <div class="modal fade CustomModal" id="EditPromotionModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="row g-3" action="<?= base_url('clinic_admin/update_participant')?>" method="POST" enctype="multipart/form-data">
                        <div class="col-md-4 col-sm-12">
                            <label class="form-label">Participant First Name <span style="color: red">*</span></label>
    ...
    <div class="row m-0 TabBar">
        <div class="Pagination">
            ...
            <a href="<?= base_url('clinic_admin/participants')?>" id="Home">&nbsp;Participant Management</a>
        </div>
...
function detailParticipant(id) {
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url('clinic_admin/Dashboard/detailsParticipant'); ?>',
        data: {u_id : uid},
        success: function(data){
...
function editParticipant(id) {
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url('clinic_admin/Dashboard/editParticipant'); ?>',
        data: {u_id : uid},
        success: function(data){
            var responce =JSON.parse(data);
...
function onDeleteParticipant() {
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url('clinic_admin/Dashboard/deleteParticipant'); ?>',
        data: {u_id : uid},
        success: function(data){
            var responce =JSON.parse(data);
...
function onAssignParticipant() {
    ...
    if(providerid != ''){
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url('clinic_admin/Dashboard/assignToParticipant'); ?>',
...
$('#email').blur(function(e) {
    ...
    if(email != '') {
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url('clinic_admin/dashboard/check_participant_email'); ?>',
            data: {email : email},
            success: function(response){
                responseData =JSON.parse(response);
                if(responseData.status == 'success') {
                    $('#resultparticipantmsg').html('<p style="color:green; margin: 8px 0 0 0">'+responseData.message+'</p>');
                    $('#addparticipantButton').prop('disabled', false);
                } else {
                    $('#resultparticipantmsg').html('<p style="color:red; margin: 8px 0 0 0">'+responseData.message+'</p>');
