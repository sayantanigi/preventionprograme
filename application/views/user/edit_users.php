                     ...
                     <div class="card-body">    
                        <form id="submitform" method="post" enctype="multipart/form-data" >
							...
							<div class="form-group mb-2">
                                ...
                                <input type="date" class="form-control" name="dob"  id="dob" required autocomplete="off" value="<?=@$dob?>">
                            </div>
							...
							<div class="form-group mb-2">
                                ...
                                <input type="number" class="form-control" name="phone"  id="phone" required autocomplete="off" value="<?=@$user->phone?>"> 
                            </div>
							...
							<div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Select Provider</label>
                                <select class="form-control" name="provider" required id="provider">
                                    <option value="">Select Provider</option>
                                    <?php
									    if(@$provider){
											foreach(@$provider as $k => $v){
												echo '<option value="'.@$v->id.'" '.((@$v->id == @$user->provider) ? 'selected' : '').'>'.@$v->name.'</option>';
											}
										}
									?>
                                </select>
							...
							<div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Insurance Provider</label>
                                <input type="text" class="form-control" name="insurance_provider"  id="insurance_provider" required autocomplete="off" value="<?=@$user->insurance_provider?>">
                            </div>
							<small id="insurance_provider_error"></small>
                        ...
                        <div class="card-body">
							...
							<div class="container">
										...
										<div class="profile clearfix">                            
											...
											<div class="user clearfix">
												<div class="avatar item" id="item">
													<img src="<?= !empty(@$user->image) ? base_url('uploads/profile/'.@$user->image.'') : base_url('uploads/unnamed.jpg'); ?>" class="img-thumbnail img-profile" id="blah">
												</div>                                
                                ...
                                <div class="col-lg-12 mb-3">
									...
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Status</h6></label><p class="text-muted" id="individual_status"><?=(@$user->status == 1) ? 'Active' : 'Inactive'?></p></div>
...
$(document).ready(function(){
	$("#submitform").on('submit', function(e){
		...
		$.ajax({
		type: 'POST',
		url: '<?php echo base_url('user/users/editIndividuals'); ?>',
		data: form_data,
		dataType:"json",
		contentType: false,
		cache: false,
 ...
 $(document).on('keyup','#fname',function(e){
	...
	$(document).on('change','#sport',function(e){
       ...
       $.ajax({
		type: 'POST',
		url: '<?php echo base_url('user/users/getSport_byId'); ?>',
		data: {sportId : sport},
		success: function(data){
	...
	$('#crop').click(function(){
		...
		canvas.toBlob(function(blob){
			...
			reader.onloadend = function(){
				...
				$.ajax({
					url:'<?php echo base_url("user/users/cropImage")?>',
					method:'POST',
	...
	$('#crop1').click(function(){
		...
		canvas.toBlob(function(blob){
			...
			reader.onloadend = function(){
				var base64data = reader.result;
				$.ajax({
					url:'<?php echo base_url("user/users/crop_CoverImage")?>',
					method:'POST',
					data:{image:base64data},
					success:function(data)
					{
...
$(document).on('change','#health_etity',function(e){
   ...
   $.ajax({
	type: 'POST',
	url: '<?php echo base_url('user/users/health_etity'); ?>',
...
$(document).on('change','#clinic',function(e){
   ...
   $.ajax({
	type: 'POST',
	url: '<?php echo base_url('user/users/get_clinic'); ?>',
	data: {clinic : clinic},
	success: function(data){
		$("#provider").html(data);
	}
