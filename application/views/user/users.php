                  ...
                  <div class="card-body">
                  	<div class="row">
	                     ...
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	<a href="<?=base_url('user/users/add')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add Participants</a>
	                     </div>
	                  </div>   	
                     <div class="">
										...
										<?php foreach ($userlist as $key => $v): ?>
										    <?php
											    // if(!empty(@$v->sub_id)){
													...
													// $subname = $sub->name;
												// }else{
													// $subname = 'Not Purchase';
												// }
												$sub = $this->db->query("select id, sub_id, end_date from transaction where payment_type = '1' and user_id = ".@$v->id." ORDER BY id DESC LIMIT 1")->row();
												if(!empty($sub)){
													if($sub->end_date >= date('Y-m-d')){
														$subscription = $this->db->query("select name from subscription where id = ".@$sub->sub_id."")->row();
														$subname = $subscription->name;
													}else{
														$subname = 'Expired Subscription';
													}
													// $subname = $this->db->query("select name from subscription where id = ".@$sub->sub_id."")->row();
													// $subname = $sub->name;
												}else{
													$subname = 'Not Purchase';
												}
											...
											<tr>
												<td><?= $key+1 ?></td>
												<td><img src="<?=!empty(@$v->image) ? base_url('uploads/profile/'.@$v->image.'') : base_url('uploads/unnamed.jpg');?>" style="width: 70%;height: 80px;border-radius: 10px;border: 3px solid #f1ecec;"></td>
												...
												<td>
												    <?php
														$data=array(
															...
															'id' => 'email_status',
															'selected'=>'Select',
															'class'=>'emailstatusClass form-control',
															'style'=>'width: 70%;margin: 0px 30px;'
														);
														$data_status = array(
															'' => 'Select',
															'1' => 'Verified',
														...
														echo form_dropdown($data, $data_status, set_value('status')); echo "<br/>";
													    if(@$v->email_verify_status == 1){
															echo '<a href="javascript:void(0)" class="btn btn-outline-success btn-sm" style="font-size: 11px;font-weight: 900;background: #198754;color: white;">Verified</a>';
															
														}else{
															echo '<a href="javascript:void(0)" class="btn btn-outline-danger btn-sm" style="font-size: 11px;font-weight: 900;background: red;color: white;">Not Verified</a>';
														}
													?>
												</td>
												<td>
												...
												<!--<td>
													<select data-value="<?=@$v->id?>" name="sub_status" row="12px" id="sub_status" class="substatusClass form-control" style="width: 70%;margin: 0px 30px;">
														<option value="">Select</option>
														<?php
														    if(!empty($user_subscription)){
															    foreach($user_subscription as $k => $v_2){
																	echo '<option value="'.@$v_2->id.'" '.((@$sub->sub_id == @$v_2->id) ? 'selected' : '').'>'.@$v_2->name.'</option>';
																}
															}
														?>
													</select><br/>
												...
												<!--<td>
													<div class="form-check mb-3 mt-3">
														<input type="checkbox" class="form-check-input small" id="customizePaystatusChange_<?=$key?>" switch="bool" value="<?= @$v->customize_payment ?>" <?= (@$v->customize_payment == 1)? 'checked':'' ?>  onchange="changeCustomizePayStatus(<?=@$v->id?>, $(this))">
														<label class="form-check-label" for="customizePaystatusChange_<?=$key?>"></label>
													</div>
												</td>-->
												
												
												<td class="text-center">
												    <a href="<?= base_url('user/users/edit/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
														<i class="fas fa-edit"></i>
													</a>
													
												    <a href="<?= base_url('user/users/view/'.@$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="View">
														<i class="fa fa-eye"></i>
													</a>
...
var adminUrl = ""
   ...
   function dealDetail(dealId) {
        var baseUrl = "<?=base_url('user/deals/detailsDeal')?>";
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= base_url('user/users/deleteUser/') ?>'+dealId
			}
		});
	}
	...
	function generateQr(userId){
		$.ajax({      
		url: '<?=base_url('user/users/qrcode')?>',       
		type: 'POST',            
		data:{userId:userId},
		success: function(data){
			if(data == '1'){
	...
	function changeDealStatus(id, thisSwitch) {      
		...
		$.ajax({         
			url: '<?php echo base_url('user/users/changestatus'); ?>',     
			type: 'POST',       
			dataType: 'json',       
			data: {         
				userId: String(id),        
	...
	function changeCustomizePayStatus(id, thisSwitch) {      
		...
		$.ajax({         
			url: '<?php echo base_url('user/users/customizePaychangestatus'); ?>',     
			type: 'POST',       
			dataType: 'json',       
			data: {         
				userId: String(id),        
...
$(document).ready(  function() {
	$(".emailstatusClass").change(function(event) {
		...
		jQuery.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>" + "user/users/update_email_status",
			dataType: 'json',
	...
	$(".substatusClass").change(function(event) {
		...
		jQuery.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>" + "user/users/update_subscription",
			dataType: 'json',
