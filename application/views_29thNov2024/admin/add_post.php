						...
						<div class="row">
							<div class="col-sm-8">
								...
								<div class="form-group mb-2 files">
									...
									<input type="hidden" class="form-control" name="userId"  id="userId"  value="<?php echo !empty($userId) ? $userId : ''; ?>">
								</div>
								...
								<div class="form-group mb-2">
								    ...
								    <textarea type="text" class="form-control editor" name="content"  id="content" autocomplete="off"></textarea>
								</div>
...
$(document).ready(function(){
	...
	$("#submitAboutus").on('submit', function(e){
		...
		$.ajax({
		type: 'POST',
		url: '<?php echo base_url('admin/post/submitImageGallery'); ?>',
		data: form_data,
		dataType:"json",
		contentType: false,
		cache: false,
		...
		success: function(data){
			// var output = $(".uploadinfobox");
			 ...
			 if(data.status == 1)
			 {
				 swal({title: "Sucess!", text: "<strong>"+data.message+"</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = "<?=base_url('admin/post/Allpost/'.@$this->uri->segment(4).'')?>"});
			 }
				// if(data.email_error != '')
				// {
