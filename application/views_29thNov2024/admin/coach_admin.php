                  ...
                  <div class="card-body">
                  	<div class="row">
	                     ...
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	<a href="<?=base_url('admin/users/addCoachadmin')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add</a>
	                     </div>
	                  </div>   	
                     <div class="">
                        <table id="datatable" class="table table-bordered dt-responsive nowrap w-100">
								   ...
								   <?php if (is_array($userlist) || is_object($userlist)) { ?>
										<?php foreach ($userlist as $key => $v): ?>
											...
											<tr>
												<td><?= $key+1 ?></td>
												<td><img src="<?=!empty(@$v->file) ? base_url('uploads/clinic/'.@$v->file.'') : base_url('uploads/noimage.jpg');?>" style="width: 70%;height: 80px;border-radius: 10px;border: 3px solid #f1ecec;"></td>
												...
												<td class="text-center">
												    <a href="<?= base_url('admin/users/editCoachadmin/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
														<i class="fas fa-edit"></i>
													</a>
													
												    <a href="<?= base_url('admin/users/viewCoachadmin/'.@$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="View">
														<i class="fa fa-eye"></i>
													</a>
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= base_url('admin/users/deleteUser_coach_admin/') ?>'+dealId
			}
		});
	}
	...
	function changeDealStatus(id, thisSwitch) {      
		...
		$.ajax({         
			url: '<?php echo base_url('admin/users/changestatus_coach_admin'); ?>',     
			type: 'POST',       
			dataType: 'json',       
			data: {         
				userId: String(id),        
