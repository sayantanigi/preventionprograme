                     ...
                     <div class="card-body">    
                        <form id="submitform" method="POST" enctype="multipart/form-data" >
                            ...
                            <div class="form-group mb-2">
                                ...
                                <input type="text" class="form-control" name="event_name"  id="event_name"  autocomplete="off" value="<?=@$event->event_name?>">
                            </div>
							...
							<div class="form-group mb-2">
                                ...
                                <textarea type="text" class="form-control editor summermote" name="event_description"  id="event_description"  autocomplete="off"><?=@$event->event_description?></textarea>
                            </div>
							...
							<div class="form-group mb-2">
								...
								<input type="text" class="form-control" name="event_address"  id="event_address"  autocomplete="off" value="<?=@$event->event_address?>">
								...
								<input type="hidden" placeholder="Near" name="event_city" id="event_city" value="<?=@$event->event_city?>">
							...
							<div class="form-group mb-2">
								<div class="form-group mb-2" >
									...
									<input type="text" class="form-control" name="event_price"  id="event_price"  autocomplete="off" value="<?=@$event->event_price?>">
								</div>
							...
							<div class="form-group mb-2">
								<div class="form-group mb-2" >
									...
									<input type="date" class="form-control" name="event_date"  id="event_date"  autocomplete="off" value="<?php echo !empty(@$event->event_date) ? date('Y-m-d',strtotime(@$event->event_date)) : '' ?>">
								</div>
							...
							<div class="form-group mb-2">
								<div class="form-group mb-2" >
									...
									<input type="time" class="form-control" name="event_time"  id="event_time"  autocomplete="off"  value="<?php echo date('h:i:s', strtotime($event->event_time)); ?>">
								</div>
							...
							<div class="form-group mb-2">
								<div class="form-group mb-2" >
									...
									<input type="hidden"  name="event_id"  id="event_id"  autocomplete="off" value="<?=@$event->event_id?>">
								</div>
										...
										if(!empty($gallerySql)){  
											if (is_array($gallerySql) || is_object($gallerySql)) {
												foreach($gallerySql as $k => $v){
													echo '
														<div class="uploadlist draggable-item preview-image preview-show-'.$v->id.'" id="preview_'.$v->id.'">
														<a href="javascript:void(0);" class="closemedia image-delete" data-no="'.$v->id.'"  id="img"><i class="fas fa-times"></i></a>
														<div class="image-zone" ><img id="pro-img-" src="'.(!empty(@$v->image) ? base_url('uploads/event/'.@$v->image.'') : '').'" class="imageall" style="width: 70%;height: 70px;border-radius: 10px;"></div>
														</div>';
												}
											}
										}
                       ...
                       <div class="card-body">
                            <div class="row">
								 ...
								 <div class="col-lg-12 mb-3">
									...
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Event Name</h6></label><p class="text-muted" id="event-name"><?=@$event->event_name?></p></div>
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Event Description</h6></label><p class="text-muted" id="event-description"><?=@$event->event_description?></p></div>
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Event Address</h6></label><p class="text-muted" id="event-address"><?=@$event->event_address?></p></div>
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Cost</h6></label><p class="text-muted" id="event-price"><?=@$event->event_price?></p></div>
									
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Event Date</h6> </label><br/><span class="text-muted" id="event-date"><?=@$event->event_date?></span></div>
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Event Time</h6> </label><br/><span class="text-muted" id="event-time"><?=@$event->event_time?></span></div>
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Status</h6></label><p class="text-muted" id="event-status"><?=(@$event->status == 1) ? 'Active' : 'Inactive' ;?></p></div>
...
$(document).ready(function(){
	$("#submitform").on('submit', function(e){
		...
		$.ajax({
		type: 'POST',
		url: '<?php echo base_url('admin/event/editEvent'); ?>',
		data: form_data,
		dataType:"json",
		contentType: false,
		cache: false,
		...
		success: function(data){
			if(data.status == 1){
				swal({title: "Sucess!", text: "<strong>"+data.message+"</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = "<?=base_url('admin/event')?>"});
			}
			if(data.status == 0){
...
$(document.body).on('click', '.image-delete' ,function(){ 
	...
	$.ajax({
		type: "POST",
		url: "<?php echo site_url('admin/event/deleteGallery');?>",
		data: {id : item},
