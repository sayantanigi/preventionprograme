                     ...
                     <div class="card-body">    
                        <form id="submitform" method="post" enctype="multipart/form-data" >
                            <div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Health Group Name</label>
                                <input type="text" class="form-control" name="group_name"  id="group_name"  autocomplete="off" value="<?=@$result->name?>">
                                <input type="hidden" class="form-control" name="id"  id="id"  autocomplete="off" value="<?=@$result->id?>">
                            </div>
							<small id="group_name_error"></small>
...
$(document).ready(function(){
	$("#submitform").on('submit', function(e){
		...
		$.ajax({
		type: 'POST',
		url: '<?php echo base_url('admin/healthgroup/updateHealthgroup'); ?>',
		...
		success: function(data){
			if(data.status == 1){
				swal({title: "Sucess!", text: "<strong>"+data.message+"</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = "<?=base_url('admin/healthgroup')?>"});
			...
			if(data.vali_error == 1){
				if(data.group_name_error != ''){
					$('#group_name_error').html(data.group_name_error);
				}else{
