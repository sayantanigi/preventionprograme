            ...
            <div class="col-xl-12">
               <div class="card custom-shadow rounded-lg border">
                  <div class="card-body">
                  	<div class="row">
	                  	<div class="col-sm-10">
	                     	<h4 class="card-title mb-4">Features List</h4>
	                     </div>
	                     
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	 <a href="<?=base_url('admin/cms/add_event_process')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add</a>
	                     </div>
	                  </div>   	
                     <div class="">
                        <table id="datatable" class="table table-bordered dt-responsive nowrap w-100">
								   ...
								   <?php if (is_array($step_process) || is_object($step_process)) { ?>
										<?php foreach ($step_process as $key => $v): ?>
										
											<tr>
												<td><?= $key+1 ?></td>
												<td><img src="<?=!empty(@$v->image) ? base_url('uploads/event_process/'.@$v->image.'') : base_url('uploads/noimage.jpg')?>" style="width: 30%;border: 4px solid #cdcdcd;border-radius: 10px;"></td>
												<td style=""><?=strip_tags(@$v->description); ?></td>
												<td>
												...
												<td class="text-center">
												    <a href="<?= base_url('admin/cms/edit_event_process/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= base_url('admin/cms/delete_event_process/') ?>'+dealId
			}
		});
	}
	...
	function changePckStatus(id, thisSwitch) {      
		...
		$.ajax({         
			url: '<?php echo base_url('admin/cms/step_process_changestatus'); ?>',     
			type: 'POST',       
			dataType: 'json',       
			data: {         
				faqId: String(id),        
		...
		.done(function(data) {  
			...
			if(newStatus == 1){
				swal({title: "Sucess!", text: "<strong>Event planning process status is Activate</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = " "});
			}else if(newStatus == 0){
				swal({title: "Sucess!", text: "<strong>Event planning process status is Inctivate</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = " "});
			}
		})
		.fail(function(data) {      
			console.log(data);       
