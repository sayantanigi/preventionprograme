                  ...
                  <div class="card-body">
                  	<div class="row">
	                     ...
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	<a href="<?=base_url('admin/globaladmin/add')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add</a>
	                     </div>
	                  </div>   	
                     <div class="">
										...
										<?php foreach ($userlist as $key => $v): ?>
											...
											<tr>
												...
												<td class="text-center">
												    <a href="<?= base_url('admin/globaladmin/edit/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= base_url('admin/globaladmin/deleteUser/') ?>'+dealId
			}
		});
	}
	...
	function changeDealStatus(id, thisSwitch) {      
		...
		$.ajax({         
			url: '<?php echo base_url('admin/globaladmin/changestatus'); ?>',     
			type: 'POST',       
			dataType: 'json',       
			data: {         
				userId: String(id),        
