                  ...
                  <div class="card-body">
                  	<div class="row">
	                     ...
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	 <a href="<?=base_url('admin/feature/add')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add</a>
	                     </div>
	                  </div>   	
                     <div class="">
                        <table id="datatable" class="table table-bordered dt-responsive nowrap w-100">
								   ...
								   <?php if (is_array($features) || is_object($features)) { ?>
										<?php foreach ($features as $key => $v): ?>
										
											<tr>
												<td><?= $key+1 ?></td>
												<td><img src="<?=!empty(@$v->image) ? base_url('uploads/feature/'.@$v->image.'') : base_url('uploads/noimage.jpg')?>"></td>
												<td style=""><?=strip_tags(@$v->heading); ?></td>
												...
												<td class="text-center">
												    <a href="<?= base_url('admin/feature/edit/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= base_url('admin/feature/delete/') ?>'+dealId
			}
		});
	}
	...
	function changePckStatus(id, thisSwitch) {      
		...
		$.ajax({         
			url: '<?php echo base_url('admin/feature/changestatus'); ?>',     
			type: 'POST',       
			dataType: 'json',       
			data: {         
				faqId: String(id),        
		...
		.done(function(data) {  
			...
			if(newStatus == 1){
				swal({title: "Sucess!", text: "<strong>Feature status is Activate</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = " "});
			}else if(newStatus == 0){
				swal({title: "Sucess!", text: "<strong>Feature status is Inctivate</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = " "});
			}
		})
		.fail(function(data) {      
			console.log(data);       
