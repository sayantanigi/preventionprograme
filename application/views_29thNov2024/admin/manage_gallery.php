            ...
            <div class="col-xl-12">
               <div class="card custom-shadow rounded-lg border">
                  <div class="card-body">
                  	<div class="row">
	                  	<div class="col-sm-10">
	                     	<h4 class="card-title mb-4">Gallery List</h4>
	                     </div>
	                     
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	 <a href="<?=base_url('admin/gallery/add/'.@$this->uri->segment(4).'')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add Post</a>
	                     </div>
	                  </div>   	
                     <div class="">
                        <table id="datatable" class="table table-bordered dt-responsive nowrap w-100">
								   ...
								   <?php if (is_array($gallery) || is_object($gallery)) { ?>
										<?php foreach ($gallery as $key => $v): ?>
											<tr>
												<td><?= $key+1 ?></td>
												<td>
												  <?php if($v->file_type == '1') { ?>
                                                    <img src="<?=!empty($v->image) ? base_url('uploads/gallery/image/'.$v->image.'') : base_url('uploads/noimage.jpg')?>" style="width: 40%;padding: 5px;border-radius: 20px;height: 120px;">
												  <?php }else{ ?>
													<video style="width: 40%;padding: 5px;border-radius: 20px;height: 120px;" controls><source  src="<?=!empty($v->image) ? base_url('uploads/gallery/image/'.$v->image.'') : base_url('uploads/noimage.jpg')?>" class="imageall" type="video/mp4"></video>
												  <?php } ?>
												</td>
												<td>
												...
												<td class="text-center">
												    ...
												    <a href="<?= base_url('admin/gallery/edit/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= base_url('admin/gallery/delete/') ?>'+dealId
			}
		});
	}
