                  ...
                  <div class="card-body">
                  	<div class="row">
	                     ...
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	<!-- <a href="<?=base_url('admin/team-coach/add')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add Coach</a>-->
	                     </div>
	                  </div>   	
                     <div class="">
                        <table id="datatable" class="table table-bordered dt-responsive nowrap w-100">
								   ...
								   <?php if (is_array($coachteamlist) || is_object($coachteamlist)) { ?>
										<?php foreach ($coachteamlist as $key => $v): ?>
											<tr>
												<td><?= $key+1 ?></td>
												<td><?= @$v->email ?></td>
												<td><?= @$v->unique_playercode ?></td>
												<?php $playerInfo = $this->Adminmodel->get_by('users', 'single', array('email' => @$v->email), '', 1);?>
												<td><?=ucfirst(@$playerInfo->first_name); ?> &nbsp; <?=ucfirst(@$playerInfo->last_name); ?></td>
												<td>
													<?php 
														if(@$v->status == 1){
															//echo "Active";
															echo '<a href="javascript:void(0)" class="badge badge-success" style="color: #fff;background: #35741b;font-size: 13px;">Active</a>';
														}else{
															//echo "Inactive";
															echo '<a href="javascript:void(0)" class="badge badge-success" style="color: #fff;background: #c31616;font-size: 13px;">Inactive</a>';
														}
													?>
												</td>
												<!--<td class="text-center">
													<a href="javascript:void(0)" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Delete"  onclick="deletePlayer(<?= @$v->id ?>,<?=$this->uri->segment(3)?>)">
													    <i class="fa fa-trash"></i>Resend
													</a>
												</td>-->
												<?php
												if(!empty(@$v->email)){
												...
												<td class="text-center">
												    <?php if(@$v->status == 1){ ?> 
													    <a href="<?=base_url('admin/individuals-players/profile/'.@$userId.'');?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Delete" >
													       View
													    </a>
													<?php }else{ ?>
														<a href="javascript:void(0)" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Delete"  onclick="resendCode(<?= @$v->id ?>)">
															Resend
														</a>
													<?php } ?>
...
var adminUrl = ""
   ...
   function dealDetail(dealId) {
	...
	function deletePlayer(dealId, coachId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= base_url('admin/users/deletePlayer/') ?>'+dealId+'/?coachId='+coachId+''
			}
		});
	}
	...
	function resendCode(id) {      
		...
		$.ajax({         
			url: '<?php echo base_url('admin/users/resendCode'); ?>',     
			type: 'POST',       
			dataType: 'json',       
			data: {         
				id: id
