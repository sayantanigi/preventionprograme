                  ...
                  <div class="card-body">
                  	<div class="row">
	                     ...
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	 <a href="<?=base_url('admin/role/add')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add</a>
	                     </div>
	                  </div>   	
                     <div class="">
										...
										<?php foreach ($result as $key => $v): ?>
											...
											<tr>
												...
												<td class="text-center">
												    <a href="<?= base_url('admin/role/edit/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= base_url('admin/role/delete/') ?>'+dealId
			}
		});
	}
	...
	function changePckStatus(id, thisSwitch) {      
		...
		$.ajax({         
			url: '<?php echo base_url('admin/role/changestatus'); ?>',     
			type: 'POST',       
			dataType: 'json',       
			data: {         
				id: String(id),        
		...
		.done(function(data) {  
			...
			if(newStatus == 1){
				swal({title: "Sucess!", text: "<strong>Role status is Activate</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = " "});
			}else if(newStatus == 0){
				swal({title: "Sucess!", text: "<strong>Role status is Inctivate</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = " "});
			}
		})
		.fail(function(data) {      
			console.log(data);       
