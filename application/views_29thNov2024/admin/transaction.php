								   ...
								   <?php if (is_array($transaction) || is_object($transaction)) { ?>
										<?php foreach ($transaction as $key => $v): ?>
											...
											<tr>
												...
												<td><?=@$sub->name ?></td>
												<td><?=@$payment_type ?></td>
												<td><?=(@$v->paid_by_admin == 1) ? 'User' : 'Admin'?></td>
												<td><?=!empty(@$v->start_date) ? date('d F, Y', strtotime(@$v->start_date)) : '';?></td>
												<td><?=!empty(@$v->end_date) ? date('d F, Y', strtotime(@$v->end_date)) : ''?></td>
