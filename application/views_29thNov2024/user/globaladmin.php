               ...
               <div class="card custom-shadow rounded-lg border">
                  <div class="card-body">
                  	<div class="row">
                        ...
                        <?php if(@$globalAdminMgnt->has_write_access == 1 || @$globalAdminMgnt->has_full_access == 1){ ?>						
							<div class="col-sm-2 text-end" style="padding-left: 54px;">
								<a href="<?=base_url('user/globaladmin/add')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add</a>
							</div>
						<?php } ?> 
										...
										<?php foreach ($userlist as $key => $v): ?>
											...
											<tr>
												...
												<td class="text-center">
												    ...
												    <?php if(@$globalAdminMgnt->has_write_access == 1 || @$globalAdminMgnt->has_full_access == 1){ ?>	
														<a href="<?= base_url('user/globaladmin/edit/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
															<i class="fas fa-edit"></i>
														</a>
													<?php } ?>	
													
													<?php if(@$globalAdminMgnt->has_full_access == 1){ ?>	 
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= base_url('user/globaladmin/deleteUser/') ?>'+dealId
			}
		});
	}
	...
	function changeDealStatus(id, thisSwitch) {      
		...
		$.ajax({         
			url: '<?php echo base_url('user/globaladmin/changestatus'); ?>',     
			type: 'POST',       
			dataType: 'json',       
			data: {         
				userId: String(id),        
...
$(document).ready(  function() {
	...
	$(".substatusClass").change(function(event) {
		...
		jQuery.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>" + "admin/globaladmin/update_subscription",
			dataType: 'json',
